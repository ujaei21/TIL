...
# =============================================================================
dat.isna().sum().sum() # 열별 결측치 확인

#(비교)
dat.isna().sum(axis=1).sum() # 행별
...
age_pr =pd.cut(Q2.Age,[0,20,30,40,50,60,Q2.Age.max()+1],
               right=False,labels=[10,20,30,40,50,60])
...
Q1.groupby('gender').forehead_ratio.var()
...
y=Q1.gender
X=Q1[['long_hair', 'forehead_width_cm', 'forehead_height_cm', 'nose_wide',
...
metrics.precision_score(pred,y_test,pos_label='Male')
...
import pandas as pd
Q4_1 = dataset4.copy()
target = Q4_1[Q4_1.LOCATION == 'KOR']
# target.corr()

Q4_out = pd.pivot_table(data=target,index='TIME',
                        values='Value',
                        aggfunc='sum')

Q4_out.reset_index(inplace=True)
Q4_out.corr()['Value']['TIME']
...
Q4_k=Q4_1[Q4_1.LOCATION == 'KOR']
Q4_j=Q4_1[Q4_1.LOCATION == 'JPN']

Q4_2=pd.merge(Q4_k,Q4_j,on=['TIME','SUBJECT'],how='left')
ttest_rel(Q4_2.Value_x,Q4_2.Value_y)
